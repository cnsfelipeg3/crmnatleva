  ...
  try {
    ...
    if (manual_transcript && manual_transcript.trim().length > 20) {
      ...
      try {
        ...
        // Fallback 1: Free YouTube Transcript API (Vercel-hosted proxy)
        if (!transcript || transcript.length < 50) {
          try {
            console.log("Trying free transcript proxy API...");
            const proxyRes = await fetch("https://youtube-transcript-api-tau-one.vercel.app/transcript", {
              method: "POST",
            ...
            if (proxyRes.ok) {
              ...
              if (proxyData?.transcript && Array.isArray(proxyData.transcript)) {
                transcript = proxyData.transcript.map((t: any) => t.text || "").join(" ").replace(/\s+/g, " ").trim();
                isAutoGenerated = true;
                console.log(`✓ Proxy API: ${transcript.length} chars`);
              } else if (typeof proxyData?.transcript === "string" && proxyData.transcript.length > 20) {
                transcript = proxyData.transcript;
                isAutoGenerated = true;
                console.log(`✓ Proxy API (string): ${transcript.length} chars`);
              }
            } else {
              console.warn(`Proxy API: HTTP ${proxyRes.status}`);
              await proxyRes.text().catch(() => {});
            }
          } catch (e: any) {
        ...
        // Fallback 2: Kome.ai transcript API
        if (!transcript || transcript.length < 50) {
          try {
        ...
        // Fallback 3: Tactiq transcript API
        if (!transcript || transcript.length < 50) {
          try {
