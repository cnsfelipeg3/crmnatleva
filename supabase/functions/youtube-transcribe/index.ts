...
async function fetchYouTubeCaptions(videoId: string): Promise<{ transcript: string; title: string; isAutoGenerated: boolean }> {
  ...
  // Try multiple cookie strategies to bypass consent screen
  ...
  for (const cookie of cookieStrategies) {
    ...
    html = await res.text();
    console.log(`Page HTML length: ${html.length}, cookie=${cookie ? "yes" : "none"}, hasCaptions=${html.includes("captionTracks")}`);
