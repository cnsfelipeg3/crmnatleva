...
/** Get video title via oembed (always works from any IP) */
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Processing YouTube video: ${videoId}, manual_transcript=${!!manual_transcript}`);
    ...
    if (manual_transcript && manual_transcript.trim().length > 20) {
      ...
      // Step 1: Try automatic caption extraction
      try {
        ...
        // Fallback: use Firecrawl (non-fatal)
        ...
        // Get title from oembed if we don't have one
        if (!videoTitle) {
          videoTitle = await getVideoTitle(videoId);
        }
