...
ALTER TABLE public.proposals
  ...
  ADD COLUMN IF NOT EXISTS internal_profit numeric;

COMMENT ON COLUMN public.proposals.internal_cost IS 'Custo interno do pacote · uso interno, nunca exibido ao cliente';
COMMENT ON COLUMN public.proposals.internal_profit IS 'Lucro real (total_value - internal_cost), editável · uso interno, nunca exibido ao cliente';
...
BEGIN
  ...
  agg AS (
    SELECT
      ...
      COALESCE(SUM(
        CASE
          WHEN internal_profit IS NOT NULL THEN internal_profit
          WHEN internal_cost IS NOT NULL AND total_value IS NOT NULL THEN total_value - internal_cost
          ELSE COALESCE(total_value, 0) * margin_avg
        END
  ...
  SELECT jsonb_build_object(
    ...
    'open_rate', CASE WHEN a.sent_count > 0 THEN (va.proposals_opened::numeric / a.sent_count) ELSE 0 END,
