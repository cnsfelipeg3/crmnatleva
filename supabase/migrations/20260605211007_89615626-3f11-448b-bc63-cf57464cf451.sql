...
BEGIN
  ...
  LOOP
    -- Merge missing fields from duplicate into master (master só ganha, nunca perde)
    ...
    -- Move payer_passenger_id em sales
    UPDATE sales SET payer_passenger_id = rec.master_id WHERE payer_passenger_id = rec.dup_id;

    -- Move companions de conversa (se a tabela existir)
    BEGIN
      EXECUTE format('UPDATE conversation_companions SET passenger_id = %L WHERE passenger_id = %L AND NOT EXISTS (SELECT 1 FROM conversation_companions cc2 WHERE cc2.conversation_id = conversation_companions.conversation_id AND cc2.passenger_id = %L)', rec.master_id, rec.dup_id, rec.master_id);
      EXECUTE format('DELETE FROM conversation_companions WHERE passenger_id = %L', rec.dup_id);
    EXCEPTION WHEN undefined_table THEN NULL;
    END;

    -- Outras FKs conhecidas (best-effort)
    BEGIN EXECUTE format('UPDATE checkin_tasks SET passenger_id = %L WHERE passenger_id = %L', rec.master_id, rec.dup_id);
...
SELECT public.deduplicate_passengers();