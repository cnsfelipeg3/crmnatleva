...
export function buildBrandSystemPrompt(): string {
  ...
  return [
    ...
    "================================",
    `Brand wordmark: lowercase "${NATLEVA_BRAND.name}" in elegant serif with a small airplane integrated into the letter "n" · NEVER recreate, draw, type or render the wordmark inside the generated artwork · the official transparent PNG is added later in post-processing.`,
    ...
    "  · No logo reference image is provided to the model. The OFFICIAL NatLeva transparent PNG is applied only in post-processing.",
    "CRITICAL · DO NOT draw, render, paint, type, sketch or recreate the natleva wordmark anywhere on the artwork. ZERO occurrences of the word 'natleva' anywhere as a graphic. The official transparent PNG logo will be STAMPED on top of the image in post-processing at the TOP-LEFT corner. If you draw any wordmark, this is a hard failure.",
    "MANDATORY · LEAVE THE TOP-LEFT ~28% width × ~15% height of the artwork as clean photographic background only · no text, no logo, no shape, no person, no decorative element, no color block. This reserved area must blend with the artwork because the final logo has transparent background and must sit directly on the photo.",
    ...
    "2. Top-left RESERVED AREA (~28% width × ~15% height): leave EMPTY · only the background photo with subtle dark gradient. The official transparent NatLeva logo will be stamped here in post-processing. DO NOT draw any wordmark, logo, text, box, plaque, rectangle or shape in this area.",
...
"7. Price block (separate Linen card OR fused with info card):",
    ...
    "· NEVER recreate, draw, type or render the NatLeva wordmark · the final transparent PNG logo is applied only after generation.",
