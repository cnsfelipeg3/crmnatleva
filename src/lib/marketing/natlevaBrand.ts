...
export function buildBrandSystemPrompt(): string {
  ...
  return [
    ...
    `Footer may show only the Instagram string "${NATLEVA_BRAND.handle}" and the WhatsApp number "${NATLEVA_BRAND.whatsapp}", never a logo or brand wordmark.`,
    `OFFICIAL CONTACT FOOTER · if a contact line appears, render EXACTLY and ONLY these two strings, character by character, with NO extra word before, after or between them (do NOT write the words "handle", "Instagram", "WhatsApp", "contato", "siga", "follow" or any label): "${NATLEVA_BRAND.handle}" separated by a mid-dot "·" from "${NATLEVA_BRAND.whatsapp}". NEVER invent, alter, shorten, mask or replace any character. NEVER output any other phone number or random word.`,
    "",
    "REQUIRED CONTENT",
