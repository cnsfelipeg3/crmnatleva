...
export function buildBrandSystemPrompt(): string {
  ...
  return [
    ...
    `Footer may show only the official handle "${NATLEVA_BRAND.handle}" and the official WhatsApp "${NATLEVA_BRAND.whatsapp}", never a logo or brand wordmark.`,
    `OFFICIAL CONTACT · render EXACTLY these strings character by character if a contact line appears: handle "${NATLEVA_BRAND.handle}" and WhatsApp "${NATLEVA_BRAND.whatsapp}". NEVER invent, alter, shorten, mask or replace any digit of the WhatsApp number. NEVER output any other phone number.`,
    "",
    "REQUIRED CONTENT",
