...
function ExpandableCard({ children, expandedContent, defaultExpanded = false }: { children: React.ReactNode; expandedContent: React.ReactNode; defaultExpanded?: boolean }) {
  ...
  return (
    <div className="rounded-2xl overflow-hidden border border-border/40 bg-card transition-shadow duration-300 hover:shadow-lg cursor-pointer" onClick={() => setExpanded(!expanded)}>
      {children}
      <AnimatePresence>
        {expanded && (
...
/* ═══ Normalize flight data (supports both flat and flight_segments formats) ═══ */
function normalizeFlightData(data: any): { origin: string; destination: string; airline: string; airline_name: string; flight_number: string; departure_time: string; arrival_time: string; terminal: string; arrival_terminal: string; duration: string; departure: string; cabin: string; baggage: string; seat: string; stops: number; notes: string; segments: any[] } {
  ...
  if (segs && Array.isArray(segs) && segs.length > 0) {
    ...
    return {
      ...
      destination: last.destination_iata || "",
      airline: first.airline_name || first.airline || "",
      airline_name: first.airline_name || first.airline || "",
  ...
  return {
    origin: data?.origin || data?.origin_iata || "",
    destination: data?.destination || data?.destination_iata || "",
    airline: data?.airline || data?.airline_name || "",
    airline_name: data?.airline_name || data?.airline || "",
    ...
    departure: data?.departure || data?.departure_date || "",
    cabin: data?.cabin || "",
    baggage: data?.baggage || "",
    seat: data?.seat || "",
    stops: data?.stops ?? 0,
...
function FlightSegmentDetail({ seg, idx }: { seg: any; idx: number }) {
  ...
  return (
    <div className={`flex items-center gap-4 py-3 ${idx > 0 ? "border-t border-dashed border-border/40" : ""}`}>
      <div className="text-center min-w-[60px]">
        <p className="text-lg font-bold text-foreground">{seg.departure_time || "—"}</p>
        <p className="text-sm font-semibold text-primary">{seg.origin_iata || "—"}</p>
      ...
      <div className="text-center min-w-[60px]">
        ...
        <p className="text-sm font-semibold text-primary">{seg.destination_iata || "—"}</p>
        ...
        expandedContent={
          <div className="px-6 pb-4 space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              {d.baggage && <DetailPill icon={<Luggage className="w-3.5 h-3.5" />} label="Bagagem" value={d.baggage} />}
            ...
            {d.notes && <p className="text-sm text-muted-foreground leading-relaxed border-l-2 border-primary/20 pl-3">{d.notes}</p>}
            {flight.description && <p className="text-sm text-muted-foreground leading-relaxed border-l-2 border-primary/20 pl-3">{flight.description}</p>}
          </div>
        }
      >
        <div className="p-6 flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center shrink-0"><Plane className="w-5 h-5 text-primary" /></div>
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1 text-sm text-muted-foreground">
              ...
              {d.departure && (() => { try { return <span>{format(new Date(d.departure.length <= 10 ? d.departure + "T00:00:00" : d.departure), "dd MMM", { locale: ptBR })}</span>; } catch { return null; } })()}
              {d.stops > 0 && <span className="text-xs bg-amber-100 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 px-1.5 py-0.5 rounded">{d.stops} conexão{d.stops > 1 ? "ões" : ""}</span>}
            </div>
          </div>
          <span className="text-xs text-primary flex items-center gap-1 shrink-0">Ver detalhes <ChevronRight className="w-3 h-3" /></span>
        </div>
      </ExpandableCard>
    </motion.div>
  );
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }}>
        ...
        expandedContent={
          <div className="px-6 pb-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {d.website && (
                <a href={d.website} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="flex items-center gap-2 rounded-xl bg-muted/40 px-3 py-2 hover:bg-muted/60 transition-colors">
                  <Globe className="w-3.5 h-3.5 text-primary" /><div><p className="text-[9px] uppercase tracking-wider text-muted-foreground/60">Website</p><p className="text-xs font-medium text-primary truncate">Visitar</p></div>
                </a>
              )}
            </div>
            {amenities.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {amenities.map((a, i) => {
                    ...
                    return <span key={i} className="inline-flex items-center gap-1.5 text-xs text-muted-foreground bg-muted/40 px-2.5 py-1.5 rounded-full">{iconKey ? amenityIcons[iconKey] : <CheckCircle className="w-3 h-3" />}{a}</span>;
                  })}
                </div>
              </div>
            )}
            ...
            {hotel.description && <p className="text-sm text-muted-foreground leading-relaxed border-l-2 border-primary/20 pl-3">{hotel.description}</p>}
      ...
      >
        ...
        <div className="p-6">
          ...
          {d.rating && <div className="flex items-center gap-1.5 mt-1"><span className="bg-primary text-primary-foreground text-xs font-bold px-1.5 py-0.5 rounded">{Number(d.rating).toFixed(1)}</span>{d.user_ratings_total && <span className="text-xs text-muted-foreground">({d.user_ratings_total} avaliações)</span>}</div>}
          {d.location && <p className="text-sm text-muted-foreground flex items-center gap-1 mt-1.5"><MapPin className="w-3.5 h-3.5" /> {d.location}</p>}
          <div className="flex flex-wrap gap-2 mt-3">
            {d.room_type && <span className="text-xs bg-muted px-2.5 py-1 rounded-full text-muted-foreground">{d.room_type}</span>}
            {d.meal_plan && <span className="text-xs bg-muted px-2.5 py-1 rounded-full text-muted-foreground">{d.meal_plan}</span>}
            {d.nights && <span className="text-xs bg-muted px-2.5 py-1 rounded-full text-muted-foreground">{d.nights} noite{d.nights > 1 ? "s" : ""}</span>}
          </div>
          <p className="text-xs text-primary flex items-center gap-1 mt-3">Ver detalhes e fotos <ChevronRight className="w-3 h-3" /></p>
        </div>
      </ExpandableCard>
    </motion.div>
  );
              ...
              <div>
                ...
                <ul className="space-y-1">
                  {(Array.isArray(d.includes) ? d.includes : [d.includes]).map((item: string, i: number) => (
                    <li key={i} className="text-xs text-muted-foreground flex items-center gap-1.5"><CheckCircle className="w-3 h-3 text-primary shrink-0" /> {item}</li>
                  ))}
                </ul>
              </div>
            )}
            {d.notes && <p className="text-xs text-muted-foreground/70 italic border-l-2 border-primary/20 pl-3">{d.notes}</p>}
          </div>
        }
      >
        ...
        <div className="p-5">
          <div className="flex items-center gap-2 mb-1"><Sparkles className="w-4 h-4 text-primary" /><h3 className="font-semibold text-foreground">{exp.title}</h3></div>
          {exp.description && <p className="text-sm text-muted-foreground line-clamp-2">{exp.description}</p>}
          <p className="text-xs text-primary flex items-center gap-1 mt-2">Ver mais <ChevronRight className="w-3 h-3" /></p>
        </div>
      </ExpandableCard>
    </motion.div>
  );
  ...
  return (
    <div className={`bg-background text-foreground ${embedded ? "rounded-xl border border-border overflow-hidden" : "min-h-screen"}`}>
      ...
      <section className={`relative ${embedded ? "h-[50vh] min-h-[320px]" : "h-screen"} flex items-end justify-center overflow-hidden`}>
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.3 }} className="relative z-10 text-center text-white pb-12 sm:pb-20 px-6 max-w-3xl">
          {proposal.client_name && <p className="text-sm tracking-[0.3em] uppercase opacity-70 mb-4">{proposal.client_name}</p>}
          <h1 className={`${embedded ? "text-3xl sm:text-4xl" : "text-4xl sm:text-5xl md:text-6xl"} font-serif font-bold leading-tight mb-4`}>{proposal.title || "Sua Viagem"}</h1>
      ...
      {/* INTRO */}
      {proposal.intro_text && (
        <section className="max-w-3xl mx-auto py-16 sm:py-20 px-6">
          <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-lg sm:text-xl leading-relaxed text-muted-foreground text-center font-light italic">
            "{proposal.intro_text}"
          </motion.p>
        </section>
      )}
      ...
      {(destinations.length > 0 || (proposal.destinations?.length > 0 && destinations.length === 0)) && (
        <section className="py-12 sm:py-16 px-6">
          <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-3xl font-serif text-center mb-12">Seus Destinos</motion.h2>
          <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {(destinations.length > 0 ? destinations : proposal.destinations.map((d: string, i: number) => ({ title: d, image_url: null, description: null, id: i }))).map((dest: any, idx: number) => (
              <motion.div key={dest.id || idx} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }} className="group rounded-2xl overflow-hidden relative h-72 cursor-pointer">
      ...
      {/* FLIGHTS */}
      {flights.length > 0 && (
        <section className="py-12 sm:py-16 px-6 bg-muted/30">
          <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-3xl font-serif text-center mb-3">Voos</motion.h2>
          <p className="text-center text-sm text-muted-foreground mb-10">Clique para ver os detalhes do voo</p>
      ...
      {hotels.length > 0 && (
        <section className="py-12 sm:py-16 px-6">
          <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-3xl font-serif text-center mb-3">Hospedagens</motion.h2>
          <p className="text-center text-sm text-muted-foreground mb-10">Clique para explorar fotos, quartos e avaliações</p>
      ...
      {experiences.length > 0 && (
        <section className="py-12 sm:py-16 px-6 bg-muted/30">
          <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-3xl font-serif text-center mb-3">Experiências Incluídas</motion.h2>
          <p className="text-center text-sm text-muted-foreground mb-10">Clique para ver mais detalhes</p>
      ...
      {(proposal.total_value || proposal.value_per_person) && (
        <section className="py-12 sm:py-16 px-6">
          <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-3xl font-serif text-center mb-12">Investimento</motion.h2>
          <div className="max-w-2xl mx-auto">
            <div className="rounded-2xl border border-border/40 bg-card p-8 text-center space-y-6">
              {proposal.value_per_person && <div><p className="text-xs uppercase tracking-[0.2em] text-muted-foreground mb-1">Valor por pessoa</p><p className="text-3xl font-serif font-bold text-foreground">{fmtCurrency(proposal.value_per_person)}</p></div>}
              {proposal.total_value && <div><p className="text-xs uppercase tracking-[0.2em] text-muted-foreground mb-1">Valor total da viagem</p><p className="text-4xl font-serif font-bold text-primary">{fmtCurrency(proposal.total_value)}</p></div>}
              {proposal.includes_text && (
                <div className="text-left border-t border-border/30 pt-4">
                  <p className="text-xs font-medium text-muted-foreground mb-2 flex items-center gap-1.5"><Shield className="w-3.5 h-3.5 text-primary" /> O que está incluso</p>
      ...
      {paymentConditions.length > 0 && (
        <section className="py-12 sm:py-16 px-6 bg-muted/30">
          <motion.h2 initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="text-3xl font-serif text-center mb-12">Condições de Pagamento</motion.h2>
          <div className="max-w-3xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-4">
            {paymentConditions.map((pc: any, idx: number) => (
              <motion.div key={idx} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }} className="rounded-xl border border-border/40 bg-card p-5 text-center">
      ...
      {!embedded && (
        <section className="py-20 px-6">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="max-w-2xl mx-auto text-center">
            ...
            <a href={`https://wa.me/?text=${encodeURIComponent(`Olá! Gostaria de reservar a viagem "${proposal.title}".`)}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-primary text-primary-foreground px-8 py-4 rounded-full text-lg font-semibold hover:bg-primary/90 transition-colors shadow-lg">
