...
export default function KpiCards({ kpiData, filtered = [], previous = [], clients = [], clientsGrowth, passengersCount, ceoMode }: Props) {
  ...
  const kpis = [
    ...
    { label: "Clientes Totais", value: (passengersCount ?? clients.length).toString(), icon: Users, color: "text-accent", change: clientsGrowth ? pctChange(clientsGrowth.newCurrent, clientsGrowth.newPrevious) : null, size: "sm", sales: filtered, extra: clientsGrowth && clientsGrowth.newCurrent > 0 ? `+${clientsGrowth.newCurrent} novos` : undefined, tooltip: passengersCount != null ? `Total de passageiros cadastrados no sistema (${passengersCount})` : (clientsGrowth?.comparisonLabel ? `Novos clientes: ${clientsGrowth.newCurrent} (${clientsGrowth.comparisonLabel}: ${clientsGrowth.newPrevious})` : undefined), footnote: clientsGrowth?.comparisonLabel },
