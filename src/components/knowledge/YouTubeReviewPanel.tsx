      ...
      if (data?.error === "CLIENT_DOWNLOAD_NEEDED" && data?.captionTracks?.length) {
        ...
        for (const track of data.captionTracks) {
          ...
          try {
            ...
            console.warn("Client caption download failed:", e);
          }
        }
        if (clientTranscript && clientTranscript.split(/\s+/).length >= 50) {
          console.log(`Client downloaded transcript: ${clientTranscript.length} chars, ${clientTranscript.split(/\s+/).length} words`);
          toast.success(`Legendas baixadas: ${clientTranscript.split(/\s+/).length} palavras. Processando...`);
          // Re-invoke with manual_transcript
          ...
          setStep("idle");
          toast.error("Não foi possível baixar as legendas. Cole a transcrição manualmente.", { duration: 6000 });
          return;
        }
      }
      if (error) throw error;
