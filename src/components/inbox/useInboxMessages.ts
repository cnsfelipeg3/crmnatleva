    ...
    const load = async () => {
      try {
        ...
        const { data: unifiedRows, error } = await (supabase
          ...
          .select("id, conversation_id, sender_type, direction, message_type, content, media_url, media_storage_url, media_status, media_mimetype, media_filename, media_size_bytes, media_failure_reason, status, timestamp, created_at, external_message_id, sender_name, sender_phone, sender_photo, is_pinned, pinned_at, is_deleted, deleted_at, is_edited, edited_at, metadata, sent_by_agent")
