...
export function translateHotelscomText(text?: string): string {
  ...
  out = out
    ...
    .replace(/\bwe have (\d+) left at this price\b/gi, (_, n) => `Restam ${pluralize(Number(n), "unidade", "unidades")} nesse preço`)
    .replace(/\bwe have (\d+) left\b/gi, (_, n) => `Restam ${pluralize(Number(n), "unidade", "unidades")}`)
